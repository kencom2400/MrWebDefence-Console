...
### PR #43レビューからの学び（Password Policy実装）

#### エラーハンドリングの適切な例外型の使用

**問題**: `ChangePasswordUseCase`でユーザーが見つからない場合に`BadRequestException`を使用していた。

**解決策**: ユーザーが見つからない場合は`NotFoundException`を使用する。

**理由**:
- HTTPステータスコードの適切な使用（404 Not Found）
- エラーの意味が明確になる
- RESTful APIのベストプラクティスに従う

#### パスワード再利用エラーメッセージの一貫性

**問題**: `ValidatePasswordPolicyUseCase`でパスワード再利用時に`errors`配列が空で、`message`のみが設定されていた。

**解決策**: パスワード再利用時も`errors`配列にエラーメッセージを含める。
...
**問題**: `PasswordPolicy` Value Objectで記号の正規表現がハードコードされていた。

**解決策**: 正規表現を定数（`SYMBOL_PATTERN`）として抽出する。
...
**問題**: `PasswordHistoryRepository.savePasswordHistory`で不要なソート処理が実行されていた。

**解決策**: `savePasswordHistory`内のソート処理を削除（`getPasswordHistory`と`deleteOldHistory`で必要な時にソートする）。
...
**問題**: E2Eテストで未使用の`userId`変数が定義されていた。
