...
#### 25-5. PR対応時のコメント追加 🟠 Critical

**問題**: PRへの対応（修正）を行った際に、PRにコメントを追加せずに修正のみを行ってしまうと、レビュアーが変更内容を把握できず、レビューが滞る可能性がある。

**解決策**: PRへの対応（修正）を行った場合、必ずPRにコメントを追加する。修正内容を明確に記載し、レビュアーが変更を確認しやすくする。
...
**参照**: PR #62 - MWD-101 WAFエンジン向けAPIトークン管理機能の詳細設計書作成（Geminiレビュー指摘）
